...
def parse_command(text):
    ...
    if not match:
        return "❌ الأمر غير صحيح أو الأرقام خارج النطاق المسموح."    
...
@ABH.on(events.NewMessage(pattern = r'(حظر عام|تقييد عام)\s+(@\w+|\d{5,10}|\d{2,3})(?:\s+(\d{5,10}|\d{2,3}))?'))
async def handle_command(event):
    ...
    if user:
        response += f"المستخدم: {user}\n"
    if user_id:
        response += f"الآيدي: {user_id}\n"
    if duration:
        response += f"المدة: {duration} دقائق\n"
